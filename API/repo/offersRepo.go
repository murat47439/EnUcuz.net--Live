...
func (of *OffersRepo) NewOffer(ctx context.Context, tx *sqlx.Tx, data *models.NewOffer) (int, error) {
	query := `INSERT INTO offers(product_id,parent_id,created_by,bidder_id,seller_id,offer_price,status, created_at, expires_at) VALUES ($1, $2, $3, $4, $5, $6, $7 ,NOW(), NOW() + INTERVAL '24 hours') RETURNING id`
	var id int
...
func (of *OffersRepo) ExistsOffer(ctx context.Context, tx *sqlx.Tx, productId, bidderId, sellerId int) (bool, error) {
	var exists bool
	query := `SELECT EXISTS(SELECT 1 FROM offers WHERE product_id = $1 AND status = $2 AND expires_at > NOW() AND bidder_id = $3 AND seller_id = $4)`
...
func (of *OffersRepo) RejectOtherOffers(ctx context.Context, tx *sqlx.Tx, id int) error {
	query := `UPDATE offers
			SET status = $1
			WHERE product_id = (
				SELECT product_id FROM offers WHERE id = $2
			)
			AND id <> $2
			AND status = $3
			AND expires_at > NOW()`
...
func (of *OffersRepo) LockAndGetPendingOffer(ctx context.Context, tx *sqlx.Tx, offerId int) (*models.OffersModel, error) {
	query := `SELECT id, product_id, bidder_id, seller_id, offer_price, status, expires_at,created_by FROM offers WHERE id = $1 AND status = $2 AND expires_at > NOW() FOR UPDATE`
...
func (of *OffersRepo) ExpireOffers(ctx context.Context) (int64, error) {
	query := `UPDATE offers SET status = $1 WHERE status = $2 AND expires_at < NOW()`
