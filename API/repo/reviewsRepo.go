...
func (rr *ReviewsRepo) AddReview(ctx context.Context, data *models.Review) error {

	exists, err := rr.ExistsReview(ctx, data.UserID, data.ProductID)
	...
	_, err = rr.db.ExecContext(ctx, query, data.UserID, data.ProductID, data.Content, data.Rating)
...
func (rr *ReviewsRepo) UpdateReview(ctx context.Context, data *models.Review) error {
	exists, err := rr.ExistsReview(ctx, data.UserID, data.ProductID)
	...
	res, err := rr.db.ExecContext(ctx, query, data.Content, data.Rating, data.Status, data.ID, data.UserID)
	if err != nil {
		return fmt.Errorf("Database error : %s", err.Error())
	}
...
func (rr *ReviewsRepo) RemoveReview(ctx context.Context, id, userID int) error {
	review, err := rr.GetReview(ctx, id)
	if err != nil {
		return err
	}
	...
	_, err = rr.db.ExecContext(ctx, query, id, userID)
...
func (rr *ReviewsRepo) GetReview(ctx context.Context, id int) (*models.Review, error) {
	if id == 0 {
...
func (rr *ReviewsRepo) GetReviews(ctx context.Context, page, prodID int) ([]*models.Review, error) {
	...
	rows, err := rr.db.QueryxContext(ctx, query, prodID, 50, offset)
	if err != nil {
...
func (rr *ReviewsRepo) GetUserReviews(ctx context.Context, user_id int) ([]*models.Review, error) {
	if user_id == 0 {
		return nil, fmt.Errorf("Invalid data")
	}
	var reviews []*models.Review
	...
	rows, err := rr.db.QueryxContext(ctx, query, user_id)
...
func (rr *ReviewsRepo) ExistsReview(ctx context.Context, userID int, prodID int) (bool, error) {
	...
	err = rr.db.GetContext(ctx, &exists, query, prodID, userID)
	if err != nil {
		return false, fmt.Errorf("Database error : %s", err.Error())
	}
	return exists, nil
}
func (rr *ReviewsRepo) ReviewStatusUpdate(ctx context.Context, id, status int) error {
	query := `UPDATE reviews SET status=$1 WHERE id = $2 AND deleted_at IS NULL`

	_, err := rr.db.ExecContext(ctx, query, status, id)
