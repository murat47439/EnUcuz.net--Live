...
func (ar *AttributeRepo) AddAttribute(ctx context.Context, data *models.Attribute) (models.Attribute, error) {
	...
	query := `INSERT INTO attributes.attributes(name) VALUES($1) RETURNING id`
...
func (ar *AttributeRepo) AddCatAttribute(ctx context.Context, data *models.CategoryAttribute) (models.CategoryAttribute, error) {
	...
	query := `INSERT INTO attributes.category_attributes(category_id,attribute_id,allow_custom,required,varianter,slicer) VALUES($1,$2,$3,$4,$5,$6) RETURNING category_id, attribute_id, allow_custom, required, varianter, slicer`
...
func (ar *AttributeRepo) AddProdAttribute(ctx context.Context, data *models.ProductAttribute, tx *sqlx.Tx) (*models.ProductAttribute, error) {
	query := `INSERT INTO attributes.product_attributes(attribute_id,product_id,value) VALUES ($1,$2,$3) RETURNING attribute_id,product_id,value`
...
func (ar *AttributeRepo) AddProdAttributes(ctx context.Context, data []models.Feature, prodID int, tx *sqlx.Tx) error {
	...
	query := fmt.Sprintf(
		"INSERT INTO attributes.product_attributes (product_id, attribute_id, value) VALUES %s",
		strings.Join(values, ","),
	)
...
func (ar *AttributeRepo) DeleteAttribute(ctx context.Context, id int) error {
	...
	query := `UPDATE attributes.attributes SET deleted_at = NOW() WHERE id = $1`
...
func (ar *AttributeRepo) GetProdAttributes(ctx context.Context, prodID int) ([]*models.ProductAttribute, error) {
	...
	query := `SELECT pa.id, pa.attribute_id,pa.product_id,pa.value, a.name AS attribute_name FROM attributes.product_attributes pa
		INNER JOIN attributes.attributes a ON a.id = pa.attribute_id
...
func (ar *AttributeRepo) GetAttributes(ctx context.Context, search string, page int) ([]models.Attribute, error) {
	...
	query := `SELECT name FROM attributes.attributes WHERE LOWER(name) LIKE $1 ESCAPE '\' AND deleted_at IS NULL LIMIT $2 OFFSET $3`
...
func (ar *AttributeRepo) GetCatAttributes(ctx context.Context, id int) ([]models.CategoryAttribute, error) {
	...
	query := `SELECT ca.id,ca.category_id,ca.attribute_id,ca.allow_custom,ca.required,ca.varianter,ca.slicer, a.name AS attribute_name, c.name AS category_name FROM attributes.category_attributes ca
	INNER JOIN attributes.attributes a ON a.id = ca.attribute_id
	INNER JOIN products.categories c ON c.id = ca.category_id
...
func (ar *AttributeRepo) CheckAttribute(ctx context.Context, name string) (bool, error) {
	...
	var exists bool
	query := `SELECT EXISTS(SELECT 1 FROM attributes.attributes WHERE name = $1)`
...
func (ar *AttributeRepo) CheckCatAttribute(ctx context.Context, atID, catID int) (bool, error) {
	...
	var exists bool
	query := `SELECT EXISTS(SELECT 1 FROM attributes.category_attributes WHERE category_id = $1 AND attribute_id = $2)`
...
func (ar *AttributeRepo) CheckProdAttribute(ctx context.Context, data *models.NewProductAttribute) (bool, error) {
	...
	query := `SELECT attribute_id FROM attributes.product_attributes WHERE product_id = $1 AND deleted_at IS NULL`
	var existingIDs []int
...
func (ar *AttributeRepo) DeleteCatAttribute(ctx context.Context, id int) error {
	...
	query := `UPDATE attributes.category_attributes SET deleted_at = NOW() WHERE id = $1`
...
func (ar *AttributeRepo) DeleteProdAttribute(ctx context.Context, id int) error {
	...
	query := `UPDATE attributes.product_attributes SET deleted_at = NOW() WHERE id = $1`
