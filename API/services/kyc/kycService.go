...
func (s *KYCService) UpdateVerificationStatus(ctx context.Context, applicantID string, status models.VerifiedStatus) error {
	...
	return s.userRepo.UpdateVerifiedStatus(ctx, user.ID, status)
}
